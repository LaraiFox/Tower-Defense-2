...
import net.laraifox.tdlwjgl.enums.EnumSettingFramerate;
...
public class Settings {
	...
	public static void initializeProgramDirectories() {
		...
		if (!tmpFile.exists())
			tmpFile.mkdir();
		tmpFile = new File(programDirectory + "/levels");
		if (!tmpFile.exists())
			tmpFile.mkdir();
		tmpFile = new File(programDirectory + "/saves");
		if (!tmpFile.exists())
			tmpFile.mkdir();
	}
	...
	public static void loadSettings() throws IllegalStateException, IOException {
		...
		value = properties.getProperty("framerate");
		if (value != null)
			framerateLimit = EnumSettingFramerate.values()[Integer.parseInt(value)];
		if (framerateLimit == null)
			framerateLimit = EnumSettingFramerate.Standard;
	}
	...
	public static void saveSettings() throws IllegalStateException, IOException {
		...
		properties.setProperty("framerate", String.valueOf(framerateLimit.ordinal()));
	...
	private static void createDefaultSettingsFile() throws IllegalStateException, IOException {
		width = EnumSettingResolution.Normal.getWidth();
		height = EnumSettingResolution.Normal.getHeight();
		framerateLimit = EnumSettingFramerate.Standard;
	...
	public static EnumSettingFramerate getFramerateLimit() {
		return framerateLimit;
	}

	public static void setFramerateLimit(EnumSettingFramerate framerateLimit) {
